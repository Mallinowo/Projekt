...
\newcommand{\grupaStudentow}{Collegium Witelona, Legnica}
...
Niniejsze sprawozdanie opisuje projekt \textbf{Altermatch} -- aplikację internetową inspirowaną serwisami randkowymi, w szczególności mechaniką kart i dopasowań rozsławioną przez Tindera. Aplikacja jest przeznaczona dla osób identyfikujących się z alternatywnymi subkulturami: \textit{emo, goth, scene, punk} i \textit{metalhead}.

Celem projektu było stworzenie kompletnej, działającej aplikacji webowej pokrywającej wszystkie 20 zagadnień kwalifikacyjnych: od architektury MVC, przez bazy danych, cache, uwierzytelnianie, lokalizację, mailing, asynchroniczne interakcje, konsumpcję i publikację API, aż po wdrożenie w kontenerach Docker.

System został zrealizowany jako monolityczna aplikacja pełnostackowa. Backend oparto na \textbf{PHP 8.2} i \textbf{Laravel 11}, frontend na szablonach \textbf{Blade}, \textbf{Tailwind CSS} i \textbf{Vanilla JavaScript}, a środowisko uruchomieniowe opisano przy pomocy \textbf{Docker Compose}. Dane przechowuje \textbf{MySQL 8}, a cache i sesje obsługuje \textbf{Redis}.
...
\hline
\autorA & TODO -- np. Backend: modele, migracje, kontrolery DiscoverController i SwipeController, logika filtrowania i cache \\
\hline
\autorB & TODO -- np. Frontend: widoki Blade, Tailwind CSS, JavaScript (discover.js, chat.js), RWD \\
\hline
\autorC & TODO -- np. Integracje: Spotify, Tenor/GIPHY, geokodowanie, Docker, mailing, REST API \\
\hline
\end{longtable}
...
Altermatch spełnia wszystkie wymagania pracy projektowej i demonstracji praktycznych umiejętności programowania aplikacji webowych. Projekt łączy klasyczną architekturę MVC z dynamicznymi interakcjami frontendowymi, trzema integracjami zewnętrznymi i pełną konteneryzacją środowiska. Zastosowany stack technologiczny (Laravel 11, MySQL, Redis, Tailwind CSS, Docker) jest reprezentatywny dla współczesnych projektów webowych i może stanowić solidną podstawę dalszego rozwoju aplikacji.
