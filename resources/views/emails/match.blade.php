...
<div class="box">
    ...
    <div class="heart">💜</div>
    <h1>{{ __('mail.match_heading', ['name' => $matched->name]) }}</h1>
    <p>{{ __('mail.match_body', ['user' => $user->name, 'matched' => $matched->name]) }}</p>
    <a href="{{ config('app.url') }}/chat" class="btn">{{ __('mail.match_cta') }}</a>
    <p style="margin-top:1.5rem;font-size:.8rem;color:#5e5880;">{{ __('mail.footer') }}</p>
</div>
</body></html>
