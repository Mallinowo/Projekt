...
<div class="h-full overflow-y-auto scrollbar-thin p-4 pb-8 md:p-8 md:pb-10 flex items-start justify-center" style="background:radial-gradient(ellipse at 50% -20%,rgba(168,85,247,.08),transparent 60%)">
    <div class="w-full bg-[#111118] border border-[#2a2a3a] rounded-2xl overflow-hidden shadow-[0_16px_45px_rgba(0,0,0,.45)]" style="max-width:860px;">
        <div class="px-5 py-4 border-b border-[#2a2a3a]">
            <div class="font-crimson text-3xl md:text-4xl font-semibold text-purple-300 leading-tight">{{ __('onboarding.title') }}</div>
            <p class="text-sm md:text-base text-[#b7b2ce] mt-1">{{ __('onboarding.subtitle') }}</p>

            <div class="mt-4">
                <div class="flex items-center justify-between gap-3">
                    <span class="text-xs font-mono tracking-wide text-[#8f88b7]">{{ __('onboarding.step') }} <span id="stepIndex">{{ $startStep }}</span>/3</span>
        ...
        <form method="POST" action="{{ route('onboarding.complete') }}" id="onboardingForm">
            @csrf
            ...
            <div class="overflow-hidden border-y border-[#2a2a3a] bg-[#0f0f17]">
                <div id="onbTrack" class="flex" style="transform:translate3d(0,0,0); align-items:flex-start;">
                    <div class="w-full flex-shrink-0 p-5 md:p-6 onb-step" data-step="1" style="opacity:1; transition:opacity .22s ease; min-height:360px;">
                        <div class="font-cinzel text-xl text-[#f0ecff] mb-1">{{ __('onboarding.photos_title') }}</div>
                        <p class="text-sm text-[#b7b2ce] mb-4">{{ __('onboarding.photos_desc') }}</p>

                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4" id="photoGrid">
                            @foreach($user->photos as $photo)
                            <div class="aspect-square rounded-lg border border-[#2a2a3a] overflow-hidden relative group" data-photo-id="{{ $photo->id }}">
                                <img src="/storage/{{ $photo->path }}" class="w-full h-full object-cover">
                                <button type="button" onclick="deletePhoto({{ $photo->id }}, this)" class="absolute top-1 right-1 w-5 h-5 bg-red-600/90 rounded-full hidden group-hover:flex items-center justify-center text-white text-xs border-none cursor-pointer">X</button>
                            </div>
                            @endforeach
                        </div>
                        ...
                        <button type="button" onclick="document.getElementById('photoInput').click()" class="bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
                            {{ __('onboarding.photos_btn') }}
                        </button>

                        <input type="hidden" id="photoCountVal" value="{{ $user->photos->count() }}">
                    ...
                    <div class="w-full flex-shrink-0 p-5 md:p-6 onb-step" data-step="2" style="opacity:.45; transition:opacity .22s ease; min-height:360px;">
                        <div class="font-cinzel text-xl text-[#f0ecff] mb-1">{{ __('onboarding.interests_title') }}</div>

                        <div class="flex gap-2">
                            <input type="text" id="interestInput" class="field-input flex-1" maxlength="30" placeholder="{{ __('onboarding.interest_placeholder') }}" onkeydown="if(event.key==='Enter'){event.preventDefault();addInterest();}">
                            <button type="button" onclick="addInterest()" class="bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">{{ __('onboarding.add') }}</button>
                        </div>
                        ...
                        <div class="mt-5 rounded-xl border border-[#2a2a3a] bg-[#13131d] p-4">
                            <div class="text-base font-semibold font-crimson tracking-normal leading-6 text-[#f0ebff] [text-shadow:none]" style="-webkit-font-smoothing:antialiased; text-rendering:optimizeLegibility;">{{ __('onboarding.interests_selected_title') }}</div>
                            ...
                            <div class="flex flex-wrap gap-2 mt-4 min-h-[44px]" id="interestsList">
                                ...
                                <span class="interest-tag interest-tag-selected" data-tag="{{ $interest }}">
                                    {{ $interest }}
                        ...
                        @error('interests')<p class="text-red-400 text-xs mt-2">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div class="w-full flex-shrink-0 p-5 md:p-6 onb-step" data-step="3" style="opacity:.45; transition:opacity .22s ease; min-height:360px;">
                        <div class="font-cinzel text-xl text-[#f0ecff] mb-1">{{ __('onboarding.artists_title') }}</div>
                        <p class="text-sm text-[#b7b2ce] mb-4">{{ __('onboarding.artists_desc') }}</p>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                            <div>
                                <label class="field-label">{{ __('onboarding.gender') }}</label>
                                <select name="gender" class="field-input" required>
                                    <option value="">— {{ __('onboarding.choose') }} —</option>
                                    <option value="female" {{ old('gender', $user->gender) === 'female' ? 'selected' : '' }}>{{ __('onboarding.gender_female') }}</option>
                                    <option value="male" {{ old('gender', $user->gender) === 'male' ? 'selected' : '' }}>{{ __('onboarding.gender_male') }}</option>
                                    <option value="nonbinary" {{ old('gender', $user->gender) === 'nonbinary' ? 'selected' : '' }}>{{ __('onboarding.gender_nonbinary') }}</option>
                                    <option value="other" {{ old('gender', $user->gender) === 'other' ? 'selected' : '' }}>{{ __('onboarding.gender_other') }}</option>
                                </select>
                                @error('gender')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label class="field-label">{{ __('onboarding.orientation') }}</label>
                                <select name="orientation" class="field-input" required>
                                    <option value="">— {{ __('onboarding.choose') }} —</option>
                                    <option value="hetero" {{ old('orientation', $user->orientation) === 'hetero' ? 'selected' : '' }}>{{ __('onboarding.orientation_hetero') }}</option>
                                    <option value="homo" {{ old('orientation', $user->orientation) === 'homo' ? 'selected' : '' }}>{{ __('onboarding.orientation_homo') }}</option>
                                    <option value="bi" {{ old('orientation', $user->orientation) === 'bi' ? 'selected' : '' }}>{{ __('onboarding.orientation_bi') }}</option>
                                    <option value="other" {{ old('orientation', $user->orientation) === 'other' ? 'selected' : '' }}>{{ __('onboarding.orientation_other') }}</option>
                                </select>
                                @error('orientation')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror
                            </div>
                        </div>

                        <label class="field-label">{{ __('onboarding.bio_label') }}</label>
                        <textarea name="bio" id="bioInput" rows="4" maxlength="500" class="field-input resize-none mb-4" placeholder="{{ __('onboarding.bio_placeholder') }}">{{ old('bio', $user->bio) }}</textarea>
                        @error('bio')<p class="text-red-400 text-xs -mt-2 mb-3">{{ $message }}</p>@enderror

                        <div class="flex flex-col sm:flex-row gap-2">
                            <input type="text" id="artistInput" class="field-input flex-1" maxlength="100" autocomplete="off" placeholder="{{ __('onboarding.artist_placeholder') }}" onkeydown="if(event.key==='Enter'){event.preventDefault();searchSpotifyArtists();}">
                            @if($spotifyEnabled)
                            <button type="button" onclick="searchSpotifyArtists()" class="bg-[#16161f] border border-[#2a2a3a] text-[#e2e0f0] px-4 py-2 rounded-lg text-sm font-semibold hover:border-purple-400 hover:text-purple-300 transition-colors w-full sm:w-auto">
                                {{ __('onboarding.spotify_search_btn') }}
                            </button>
                            @endif
                        </div>

                        <div class="mt-4 rounded-xl border border-[#2a2a3a] bg-[#13131d] p-4">
                            <div class="text-sm font-semibold text-[#d6d1ee]">{{ __('onboarding.spotify_title') }}</div>
                            @if($spotifyEnabled)
                            <p id="spotifyInfo" class="text-sm text-[#9b95be] mt-1">{{ __('onboarding.spotify_search_only_desc') }}</p>
                            <div id="spotifyResults" class="flex flex-wrap gap-2 mt-3"></div>
                            @else
                            <p class="text-sm text-[#9b95be] mt-1">{{ __('onboarding.spotify_not_configured') }}</p>
                            @endif
                        </div>

                        <div class="mt-5 rounded-xl border border-[#2a2a3a] bg-[#13131d] p-4">
                            <div class="text-sm font-semibold text-[#d6d1ee]">{{ __('onboarding.artists_selected_title') }}</div>
                            <p class="text-sm text-[#9b95be] mt-1">{{ __('onboarding.artists_selected_desc') }}</p>
                            <div class="flex flex-wrap gap-2 mt-4 min-h-[44px]" id="artistsList">
                                @foreach($artistValues as $artist)
                                <span class="interest-tag" data-tag="{{ $artist }}">
                                    {{ $artist }}
                                    <button type="button" class="text-[#8d87b6] hover:text-red-400 ml-1" onclick="removeTag(this.parentElement)">X</button>
...
const PHOTO_HINT_TEMPLATE = @json(__('onboarding.photos_hint', ['count' => '__COUNT__']));
const INTEREST_HINT_TEMPLATE = @json(__('onboarding.interests_count', ['count' => '__COUNT__']));
const STEP_NAMES = {
    1: @json(__('onboarding.photos_title')),
    2: @json(__('onboarding.interests_title')),
    3: @json(__('onboarding.artists_title')),
};
const NEED_PHOTO = @json(__('onboarding.need_photo'));
const MIN_INTERESTS = @json(__('onboarding.min_interests'));
const SPOTIFY_ENABLED = @json($spotifyEnabled);
const SPOTIFY_SEARCH_URL = @json(route('spotify.search'));
const SPOTIFY_SEARCH_MIN = @json(__('onboarding.spotify_search_min'));
const SPOTIFY_SEARCHING = @json(__('onboarding.spotify_searching'));
const SPOTIFY_NOT_FOUND = @json(__('onboarding.spotify_not_found'));
const SPOTIFY_ERROR = @json(__('onboarding.spotify_error'));
const SPOTIFY_ADDED_TEMPLATE = @json(__('onboarding.spotify_added', ['name' => '__NAME__']));
